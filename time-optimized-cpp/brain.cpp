...
inline void gen_random_index(std::vector<int> &index, int range, int size)
{
    ...
    static std::mt19937 generator{rd()}; //据说生成速度比rand快
    // static std::uniform_int_distribution<> uniform_distrib(0, range - 1); //[0,range) https://en.cppreference.com/w/cpp/numeric/random/uniform_int_distribution
...
inline void padding_col(unordered_map<int, deque<float>> &mat, int new_size)
{
    ...
    for (auto &it : mat)
    {
        ...
        // it->second.insert(it->second.end(), pad_size, 0); //在向量尾部插入pad_size个0
        ...
        // cout << "after padding_col 1 : mat[" << it->first << "].size=" << it->second.size() << endl;
        // cout << "after padding_col 2 : mat[0].size=" << mat[0].size() << endl;
        // cout << "after padding_col 3 : mat[it.first].size=" << mat[it->first].size() << endl;
    }
 ...
 */
void gen_binomial_mat(unordered_map<int, deque<float>> &mat_out, int k, float p, pair<int, int> size)
{
    ...
    {
        mat_out[i] = null_vec;
        gen_binomial_vec(mat_out[i], k, p, col);
    }
}
...
void Brain::add_stimulus(string name0, int k0) // k0是刺激区域在本轮中，激活的神经元数量
{
    ...
    for (auto &areas_it : this->areas)
    {
        ...
        {
            null_vec.resize(area_ptr->k, 0); //这里k的位置其实是填各area已有记录的神经元数量，不妨先填为k
...
void Brain::add_area(string name0, int n0, int k0, float beta0)
{
    ...
    {
        stimuli_connectomes_it.second[name0] = null_vec; //这里的np.empty不是清空，而是初始化
        this->areas[name0].stimulus_beta[stimuli_connectomes_it.first] = beta0;
    }

    unordered_map<string, unordered_map<int, deque<float>>> new_connectomes;
    int other_area_size = 0;
    unordered_map<int, deque<float>> null_mat; //会出现空指针，需要初始化
    string key;
    Area *area_ptr;
    for (auto &areas_it : this->areas)
    {
        ...
        generate_null_mat(null_mat, k0, other_area_size); //(k0, other_area_size),(0, other_area_size) 会在之后的每次新增中用padding补充并初始化
        // cout<<"null_mat1=("<<null_mat.size()<<","<<null_mat[0].size()<<")"<<endl;
        new_connectomes[key] = null_mat; //包括了自环的情况

        if (key != name0)
        {
            generate_null_mat(null_mat, other_area_size, k0); //不能加，否则后面的padding会出错
            // cout<<"null_mat2=("<<null_mat.size()<<","<<null_mat[0].size()<<")"<<endl;
...
void Brain::add_explicit_area(string name0, int n0, int k0, int beta0)
{
    ...
    Area *area_ptr;
    int other_n;
    unordered_map<int, deque<float>> null_mat;
    unordered_map<string, unordered_map<int, deque<float>>> new_connectomes;
    for (auto &areas_it : this->areas)
    {
        key = areas_it.first;
...
int Brain::project_into(Area &area0, deque<string> from_stimuli, deque<string> from_areas, bool verbose)
{
    ...
    unordered_map<int, deque<float>> *connectome0_ptr;
    for (const auto &from_area : from_areas)
    {
        ...
        for (const auto &w0 : this->areas[from_area].winners)
        {
            // padding_row(this->connectomes[from_area][name0], w0 + 1); //我自己加的,应该是哪里有问题导致row不足

            for (int i = 0; i < area0.w; ++i)
            {
                // cout << "connectomes[" << from_area << "][" << name0 << "].at(" << w0 << ").at(" << i << ")" << endl;
                prev_winner_inputs[i].second += (*connectome0_ptr).at(w0).at(i); // Attempt to take address of value not located in memory.
            }
        }
    } // end for from_areas
    if (verbose == true)
    ...
    flat_set<int> new_winner_indices;
    new_winner_indices.reserve(area0.k); //预先分配空间
    for (int i = 0; i < area0.k; ++i)
    {
        new_winner_indices.insert((*all_potential_winners_ptr)[i].first);
    }
    ...
    area0.new_winners = new_winner_indices; // faster and because do not use new_winner_indices any more
    area0.new_w = area0.w + num_first_winners;

    // For experiments with a "fixed" assembly in some area.
    if (area0.fixed_assembly == true)
    {
        area0.new_winners = area0.winners;
        area0.new_w = area0.w;
    ...
    for (const auto &stim : from_stimuli)
    {
        if (num_first_winners > 0)
        {
            // null_vec.resize(num_first_winners, 0.0);
    ...
    float area_to_area_beta = 0;
    for (const auto &from_area : from_areas)
    {
        from_area_w = this->areas[from_area].w;

        from_area_winners_ptr = &(this->areas[from_area].winners); //这里需要读取其他area的winner，只读不写，造成数据冲突
        // from_area_winners.clear();
        // from_area_winners.reserve(this->areas[from_area].winners.size());
        // from_area_winners.insert(this->areas[from_area].winners.begin(), this->areas[from_area].winners.end());

        // cout << "before padding_col: connectomes[" << from_area << "][" << name0 << "][0].size = " << connectomes[from_area][name0][0].size() << endl;
        // cout << "padding_col: connectomes[" << from_area << "][" << name0 << "],num_first_winners=" << num_first_winners << endl;
        padding_col(this->connectomes[from_area][name0], area0.w + num_first_winners);
        // cout << "after padding_col: connectomes[" << from_area << "][" << name0 << "][0].size = " << connectomes[from_area][name0][0].size() << endl;
        ...
        area_to_area_beta = area0.area_beta[from_area];
        if (this->no_plasticity == true)
        {
        ...
        for (const auto &i : area0.new_winners)
        {
            for (const auto &j : (*from_area_winners_ptr))
            {
                ...
                this->connectomes[from_area][name0].at(j).at(i) *= (1.0 + area_to_area_beta);
            }
        }
        if (verbose == true)
        {
            // cout << "Connectome of " + from_area + " to " + name0 + " is now:\n";
            print_mat(this->connectomes[from_area][name0]);
        }
    ...
    string other_area;
    unordered_map<int, deque<float>> *temp_mat_ptr;
    int columns = 0; // number of columns
    for (const auto &areas_it : this->areas)
    {
        other_area = areas_it.first;
        temp_mat_ptr = &(this->connectomes[other_area][name0]);

        // cout << "other_area=" << other_area << endl;
        if (std::find(from_areas.begin(), from_areas.end(), other_area) == from_areas.end())
        { // if other_area not in from_areas
            // cout << "before padding_col: connectomes[" << other_area << "][" << name0 << "][0].size = " << connectomes[other_area][name0][0].size() << endl;
            // cout << "padding_col: connectomes[" << other_area << "][" << name0 << "],num_first_winners=" << num_first_winners << endl;
            padding_col(*temp_mat_ptr, area0.new_w);
            // cout << "after padding_col: connectomes[" << other_area << "][" << name0 << "][0].size = " << connectomes[other_area][name0][0].size() << endl;
            // padding_row(*temp_mat_ptr, this->areas[other_area].w); //这行是我自己加的

            // cout << "connectomes[" << other_area << "][" << name0 << "]=(" << connectomes[other_area][name0].size() << "," << connectomes[other_area][name0][0].size() << ")" << endl;
            for (int j = 0; j < this->areas[other_area].w; ++j)
            {
                for (int i = area0.w; i < area0.new_w; ++i)
                {
        ...
        } // end if other_area not in from_areas

        temp_mat_ptr = &(this->connectomes[name0][other_area]);
        // cout << "before padding_row: connectomes[" << other_area << "][" << name0 << "].size = " << connectomes[other_area][name0].size() << endl;
        // cout << "padding_row: connectomes[" << other_area << "][" << name0 << "],num_first_winners=" << num_first_winners << endl;
        padding_row(*temp_mat_ptr, area0.new_w);
        // cout << "after padding_row: connectomes[" << other_area << "][" << name0 << "].size = " << connectomes[other_area][name0].size() << endl;

        columns = (*temp_mat_ptr)[0].size();
        for (int i = area0.w; i < area0.new_w; ++i)
        {
            for (int j = 0; j < columns; ++j)
            {
                (*temp_mat_ptr).at(i).at(j) = binomial_distrib(generator); // np.random.binomial(1, self.p);
            }
        }

        if (verbose == true)
        {
            cout << "Connectome of " + name0 + " to " + other_area + " is now:";
            print_mat(this->connectomes[name0][other_area]);
            // cout << "Debug project_into:connectomes[" + name0 + "][" + other_area + "].size=" << temp_mat_ptr->size() << endl;
        }
    } // end for other_area in areas
